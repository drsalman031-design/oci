...
export default function GoogleDriveSync({ 
  ...
  const getDriveKey = (baseKey: string) => {
    return userEmail ? `${baseKey}_${userEmail.toLowerCase().trim()}` : baseKey;
  };
  ...
  useEffect(() => {
    async function loadSyncState() {
      try {
        const connected = await AsyncStorage.getItem(getDriveKey('oci_gdrive_connected')) === 'true';
        const auto = await AsyncStorage.getItem(getDriveKey('oci_gdrive_auto_sync')) === 'true';
        const lastTime = await AsyncStorage.getItem(getDriveKey('oci_gdrive_last_backup')) || 'Never';
        const bCount = Number(await AsyncStorage.getItem(getDriveKey('oci_gdrive_backed_count'))) || 0;
        ...
        if (connected) {
          addLog(`Google Drive App Folder API connected securely for ${userEmail}.`, 'success');
        }
      } catch (err) {
  ...
  }, [visible, assessments.length, userEmail]);
  ...
  const handleConnectGoogle = () => {
    ...
    setTimeout(async () => {
      try {
        ...
        await AsyncStorage.setItem(getDriveKey('oci_gdrive_connected'), 'true');
        await AsyncStorage.setItem(getDriveKey('oci_gdrive_auto_sync'), 'true');
        setAutoSync(true);
        ...
        Alert.alert("Google Drive Connected", `Successfully linked to ${userEmail || 'drsalman031@gmail.com'}. Secure encryption is now active.`);
      } catch (e) {
        setIsConnecting(false);
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            setBackedUpCount(0);
            setStorageUsed('0 KB');
            await AsyncStorage.removeItem(getDriveKey('oci_gdrive_connected'));
            await AsyncStorage.removeItem(getDriveKey('oci_gdrive_auto_sync'));
            await AsyncStorage.removeItem(getDriveKey('oci_gdrive_backup_payload'));
  ...
  const handleToggleAutoSync = async (value: boolean) => {
    ...
    setAutoSync(value);
    await AsyncStorage.setItem(getDriveKey('oci_gdrive_auto_sync'), value ? 'true' : 'false');
  ...
  const finalizeBackup = async () => {
    try {
      ...
      setBackedUpCount(assessments.length);
      await AsyncStorage.setItem(getDriveKey('oci_gdrive_last_backup'), timestamp);
      await AsyncStorage.setItem(getDriveKey('oci_gdrive_backed_count'), String(assessments.length));
      ...
      await AsyncStorage.setItem(getDriveKey('oci_gdrive_backup_payload'), JSON.stringify(payload));
  ...
  const triggerRestore = async () => {
    ...
    const cloudPayloadStr = await AsyncStorage.getItem(getDriveKey('oci_gdrive_backup_payload'));
    if (!cloudPayloadStr) {

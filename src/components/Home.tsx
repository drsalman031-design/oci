  ...
  return (
    ...
    >
      <View style={tw`space-y-8 mt-6 max-w-4xl mx-auto w-full`}>
        ...
        <View style={tw`bg-white rounded-[24px] border border-slate-100 p-8 shadow-sm space-y-6`}>
          <View style={tw`space-y-2`}>
            <View style={tw`flex-row items-center bg-teal-50 border border-teal-100 px-3 py-1 rounded-full self-start`}>
              <Sparkles size={12} color="#0D9488" style={tw`mr-1.5`} />
              <Text style={tw`text-[#0D9488] text-[10px] font-black uppercase tracking-wider`}>
          ...
          <Pressable
            ...
            style={({ pressed }) => [
              tw`flex-row items-center justify-center bg-teal-600 py-4 px-6 rounded-2xl shadow-sm`,
        ...
        <View style={tw`space-y-4`}>
          ...
          <View style={tw`flex-row flex-wrap gap-4`}>
            {/* Total Patients */}
            <View style={tw`flex-1 min-w-[45%] bg-white border border-slate-100 rounded-[20px] p-5 shadow-sm space-y-2`}>
              <Text style={tw`text-[10px] font-bold text-slate-400 uppercase tracking-wider`}>Total Patients</Text>
              <Text style={tw`text-2xl font-black text-slate-900`}>{totalReports}</Text>
            </View>

            {/* Reports Generated */}
            <View style={tw`flex-1 min-w-[45%] bg-white border border-slate-100 rounded-[20px] p-5 shadow-sm space-y-2`}>
              <Text style={tw`text-[10px] font-bold text-slate-400 uppercase tracking-wider`}>Reports Generated</Text>
              <Text style={tw`text-2xl font-black text-slate-900`}>{totalReports}</Text>
            </View>

            {/* Average OCI Score */}
            <View style={tw`flex-1 min-w-[45%] bg-white border border-slate-100 rounded-[20px] p-5 shadow-sm space-y-2`}>
              <Text style={tw`text-[10px] font-bold text-slate-400 uppercase tracking-wider`}>Average OCI Score</Text>
              <Text style={tw`text-2xl font-black text-slate-900`}>{avgOciVal}%</Text>
            </View>

            {/* High Complexity Cases */}
            <View style={tw`flex-1 min-w-[45%] bg-white border border-slate-100 rounded-[20px] p-5 shadow-sm space-y-2`}>
              <Text style={tw`text-[10px] font-bold text-slate-400 uppercase tracking-wider`}>High Complexity Cases</Text>
              <Text style={tw`text-2xl font-black text-slate-900`}>{highComplexityCount}</Text>
            </View>
          </View>
        </View>
          ...
          <View style={tw`flex-row justify-between items-center`}>
            ...
            {totalReports > 0 && (
              <Pressable onPress={onViewHistory}>
                <Text style={tw`text-xs font-bold text-teal-600 uppercase`}>View All →</Text>
              </Pressable>
            )}
          </View>

          {/* Quick Search */}
          <View style={tw`flex-row items-center bg-white border border-slate-200 rounded-xl px-4 py-2.5 shadow-sm`}>
            ...
            <TextInput
              ...
              style={tw`flex-1 text-slate-800 text-xs h-9 p-0`}
            />
          </View>
          ...
          <View style={tw`space-y-3`}>
            {filteredPatients.length > 0 ? (
              filteredPatients.slice(0, 5).map((item) => (
                <View 
                  key={item.id}
                  style={tw`bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex-row items-center justify-between`}
                >
                  <View style={tw`flex-row items-center space-x-4`}>
                    <View style={tw`w-10 h-10 rounded-xl bg-teal-50 border border-teal-100 items-center justify-center`}>
                      <Text style={tw`text-xs font-bold text-[#0D9488]`}>
                        {item.patientDetails.diagnosis === 'Class I' ? 'I' : item.patientDetails.diagnosis === 'Class II' ? 'II' : 'III'}
                      </Text>
                    </View>
                    <View>
                      <Text style={tw`text-xs font-black text-slate-900`}>{item.patientDetails.name}</Text>
                  ...
                  <View style={tw`items-end`}>
                    <Text style={tw`text-sm font-black text-teal-600`}>{getActiveScore(item)}%</Text>
                    <Text style={tw`text-[9px] text-slate-400 uppercase font-bold tracking-wider`}>OCI Score</Text>
                  </View>
                </View>
              ))
            ) : (
              <View style={tw`bg-white rounded-2xl p-8 border border-slate-100 items-center justify-center space-y-2`}>
                <Text style={tw`text-slate-400 text-xs text-center`}>
        ...
        <View style={tw`pt-12 pb-16 items-center space-y-3 border-t border-slate-200 mt-4`}>
          <Text style={tw`text-[9px] text-slate-400 font-mono text-center uppercase tracking-widest`}>
            OCI Analyzer™ • SECURE HIPAA-COMPLIANT CLINICAL PROTOCOLS
          </Text>
          <View style={tw`flex-col items-center space-y-1`}>
            <Text style={tw`text-[10px] text-slate-400 font-bold uppercase tracking-widest text-center`}>
              Developed & Innovated by
            </Text>
            <Text style={tw`text-xs font-black text-teal-600 tracking-wider text-center uppercase`}>
